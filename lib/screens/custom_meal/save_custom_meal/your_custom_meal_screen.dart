...
class YourCustomMealScreen extends StatefulWidget {
  const YourCustomMealScreen({super.key});

  @override
  State<YourCustomMealScreen> createState() => _YourCustomMealScreenState();
}

class _YourCustomMealScreenState extends State<YourCustomMealScreen>
  ...
  void initState() {
    ...
    provider.loadAvailableItems().then((_) {
      // After loading available items, load from custom meal if editing
      if (provider.isEditing && provider.editingMeal != null) {
        provider.loadFromCustomMeal(provider.editingMeal!);
      }
    });
  }
                          ...
                          onPressed: customMealProvider.selection.protein != null
                              ? () {
                                  ...
                                  if (!authProvider.isAuthenticated) {
                                    ...
                                    final id = customMealProvider.editingMeal?.id ?? 0;  // Get id from editingMeal
                                    GoRouter.of(context).push('/saved-custom-meal/review/$id');  // Use correct id
                                  }
                                }
                              : null,
                          child: const Text(
