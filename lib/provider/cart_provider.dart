...
class CartProvider with ChangeNotifier {
  ...
  Future<void> addMealToCart(int customerId, {
    ...
    if (_cart == null) {
      _cart = Cart(cartItems: [], totalAmount: 0.0, totalItems: 0, totalQuantity: 0);
    }

    final existingItemIndex = _cart!.cartItems.indexWhere((item) => item.menuMealId == menuMealId);
    ...
    _syncAddToServer(customerId, menuMealId, quantity, unitPrice, title, description, image, calories, protein, carbs, fat);
  }
  ...
  Future<void> increaseQuantity(int customerId, int cartItemId) async {
    ...
    final itemIndex = _cart!.cartItems.indexWhere((item) => item.id == cartItemId);
    if (itemIndex != -1) {
  ...
  Future<void> _syncAddToServer(int customerId, int menuMealId, int quantity, double unitPrice, String title, String description, String image, int calories, double protein, double carbs, double fat) async {
    try {
  ...
  CartItem? getCartItemByMenuMealId(int menuMealId) {
    ...
    try {
      return _cart!.cartItems.firstWhere((item) => item.menuMealId == menuMealId);
    } catch (e) {
      return null;
    }
  }
  ...
  // Load cart từ local storage trước, nếu không có thì từ server
  ...
  //     // Nếu local null (chưa có data), load từ server và lưu local
  //     if (_cart == null) {
  //       try {
  //         _cart = await CartService().getCartByCustomerId(customerId);
  //         await _saveCartToLocal();
  //       } catch (e) {
  //         // Nếu server cũng không có hoặc lỗi, tạo cart trống
  ...
  //     // Sync với server trong background (tùy chọn, để đảm bảo dữ liệu mới nhất)
  //     _syncWithServerInBackground(customerId);
  //   } catch (e) {
  //     _error = e.toString();
  //     // Nếu có lỗi và chưa có cart, tạo cart trống
  //     if (_cart == null) {
  //       _cart = Cart(cartItems: [], totalAmount: 0.0, totalItems: 0, totalQuantity: 0);
  //     }
  //   } finally {
